...
class SignTypedMessageMethodTests: XCTestCase {
    ...
    func testRequestURLWithDEXON() {
        ...
        XCTAssertEqual(url.absoluteString, "dexon-wallet://sign-typed-message?id=1122&blockchain=dexon&callback=example&name=dapp&message=RGVrdVNhbiBpcyBjb29s&from=" + address)
    }
    ...
    func testRequestURLWithEthereum() {
        ...
        let method = SignTypedMessageMethod(message: message.data(using: .utf8)!, fromAddress: address) { _ in }
        let url = method.requestURL(scheme: "dexon-wallet", queryItems: items)
        XCTAssertEqual(url.absoluteString, "dexon-wallet://sign-typed-message?id=1122&blockchain=ethereum&callback=example&name=dapp&message=RGVrdVNhbiBpcyBjb29s&from=" + address)
    }
    ...
    func testInitWithURLComponents() {
        ...
        let components = URLComponents(
            string: "dexon-wallet://sign-typed-message?id=1122&blockchain=dexon&callback=example&name=dapp" +
