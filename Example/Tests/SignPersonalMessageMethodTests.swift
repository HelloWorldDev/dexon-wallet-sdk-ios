...
class SignPersonalMessageMethodTests: XCTestCase {
    ...
    func testRequestURLWithDEXON() {
        ...
        let method = SignPersonalMessageMethod(message: message.data(using: .utf8)!, fromAddress: address) { _ in }
        let url = method.requestURL(scheme: "dexon-wallet", queryItems: items)
        XCTAssertEqual(url.absoluteString, "dexon-wallet://sign-personal-message?id=1122&blockchain=dexon&callback=example&name=dapp&message=RGVrdVNhbiBpcyBjb29s&from=" + address)
    }
    ...
    func testRequestURLWithEthereum() {
        ...
        let method = SignPersonalMessageMethod(message: message.data(using: .utf8)!, fromAddress: address) { _ in }
        let url = method.requestURL(scheme: "dexon-wallet", queryItems: items)
        XCTAssertEqual(url.absoluteString, "dexon-wallet://sign-personal-message?id=1122&blockchain=ethereum&callback=example&name=dapp&message=RGVrdVNhbiBpcyBjb29s&from=" + address)
    }
    ...
    func testInitWithURLComponents() {
        ...
        let components = URLComponents(
            string: "dexon-wallet://sign-personal-message?id=1122&blockchain=dexon&callback=example&name=dapp" +
                "&message=RGVrdVNhbiBpcyBjb29s" +
                "&from=" + address)!
        let method = SignPersonalMessageMethod(components: components)
    ...
    func testHandleSucceedCallback() {
        ...
        let method = SignPersonalMessageMethod(message: Data()) { (result) in
