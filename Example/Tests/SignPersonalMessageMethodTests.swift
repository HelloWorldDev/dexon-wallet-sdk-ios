...
class SignPersonalMessageMethodTests: XCTestCase {
    ...
    func testRequestURLWithDEXON() {
        ...
        XCTAssertEqual(url.absoluteString, "dekusan://sign-personal-message?id=1122&blockchain=dexon&callback=example&name=dapp&message=RGVrdVNhbiBpcyBjb29s&from=" + address)
    }
    ...
    func testRequestURLWithEthereum() {
        ...
        let method = SignPersonalMessageMethod(message: message.data(using: .utf8)!, fromAddress: address) { _ in }
        let url = method.requestURL(scheme: "dekusan", queryItems: items)
        XCTAssertEqual(url.absoluteString, "dekusan://sign-personal-message?id=1122&blockchain=ethereum&callback=example&name=dapp&message=RGVrdVNhbiBpcyBjb29s&from=" + address)
    }
    ...
    func testInitWithURLComponents() {
        ...
        let components = URLComponents(
            string: "dekusan://sign-personal-message?id=1122&blockchain=dexon&callback=example&name=dapp" +
