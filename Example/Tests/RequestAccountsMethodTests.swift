...
class RequestAccountsMethodTests: XCTestCase {
    ...
    func testRequestURLWithDEXON() {
        ...
        XCTAssertEqual(url.absoluteString, "dexon-wallet://request-accounts?id=1122&blockchain=dexon&callback=example&name=dapp")
    }
    ...
    func testRequestURLWithEthereum() {
        ...
        let method = RequestAccountsMethod { _ in }
        let url = method.requestURL(scheme: "dexon-wallet", queryItems: items)
        XCTAssertEqual(url.absoluteString, "dexon-wallet://request-accounts?id=1122&blockchain=ethereum&callback=example&name=dapp")
    }

    func testInitWithURLComponents() {
        let components = URLComponents(string: "dexon-wallet://request-accounts?id=1122&blockchain=dexon&callback=example&name=dapp")!
    ...
    func testHandleSucceedCallback() {
        ...
        let components = URLComponents(string: "example://dexon-wallet?id=1122&address=" + data.base64EncodedString())!
